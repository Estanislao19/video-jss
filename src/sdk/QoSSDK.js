...
export var SDK = function(playerComponent) {
  ...
  var getConnectionType = function() {
    ...
    getUser = function() {
      ...
      var userId = aws.getUserId(); // se refiere AWSConnector
    ...
    initialize = function(videoId) { // inicalizacion
      try{
      console.log("In initialize :",videoId);
      currentVideo = resetVideo(videoId); // le paso las funciones e arriba
      ...
      console.log('error de intialize',error)
    }
    },
  ...
  var Metric = function(MetricType, MetricValue) {
      ...
      var metric = {
        ...
        ...MetricValue,
        ...commonAttr,
        TimeStamp: Date.now()//La función NOW devuelve una indicación de fecha y hora basada en la lectura del reloj cuando se ejecuta la sentencia de SQL en el servidor actual.
  ...
  //-----Definición de los controladores de funciones de eventos del reproductor ------
  ...
  var play = function(playlistType) {
    console.log("In play with playlistType ",playlistType);
      ...
      //establecer si el video que se está reproduciendo es VOD o Live
